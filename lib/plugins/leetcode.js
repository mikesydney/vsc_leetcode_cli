...
plugin.signin = function(user, cb) {
  ...
  request(config.sys.urls.login, function(e, resp, body) {
    ...
    request.post(opts, function(e, resp, body) {
      if (e) return cb(e);
      if (resp.statusCode !== 302) return cb('invalid password? try to use cookie?');
...
function parseCookie(cookie, body, cb) {
  ...
  if (reSessionResult === null || reCsrfResult === null) {
    ...
    console.log('Please generate the cookie using the format of: LEETCODE_SESSION={{LEETCODE_SESSION}};csrftoken={{csrftoken}};');
    console.log('LEETCODE_SESSION and csrftoken can be taken from browser cookies');
    console.log('LEETCODE_SESSION is: ' + String(reSessionResult));
    console.log('csrftoken is: ' + String(reCsrfResult));
